...
sel<-bio.matrix_sel$month%in%(5:10)
bio.matrix_sel<-bio.matrix_sel[sel,]
...
# other methods should be used for validation: e.g. leve-one-out or cross-validation
...
# un pò strano il modo di definire il numero di knots, sceglierei un numero di knots per anno (e.g. 6) moltiplicati per i numeri degli anni 5
...
bseas <- crossbasis(mydf$weekn, lag=0, 
          argvar=list(fun="ns", df=4), group=mydf$ID)
...
# Build formula with those columns
form <- as.formula(paste("eggs ~", paste(colnames( cbind(cbTemp_df,  bseas_df, year=mydf_ext$year)), collapse = " + "))) # btrend_df,
