...
bio.matrix_sel <- bio.matrix_cal%>%
  dplyr::arrange(ID, year, week) %>% 
  dplyr::group_by(ID) %>%
  dplyr::mutate(nweek = 1:n()) %>%
  dplyr::mutate(weekn = as.numeric(week)) %>%
  dplyr::ungroup()

# Confirm nweek is consecutive and starts at 1 per trap
bio.matrix_sel %>%
  dplyr::group_by(ID) %>%
  dplyr::summarise(
    nweek_min  = min(nweek),
    nweek_max  = max(nweek),
    n_rows     = dplyr::n(),
    is_consecutive = (nweek_max - nweek_min + 1) == n_rows,
    .groups = "drop"
  ) %>%
  dplyr::summarise(
    all_start_at_1     = all(nweek_min == 1),
    all_consecutive    = all(is_consecutive),
    min_nweek_max      = min(nweek_max),
    max_nweek_max      = max(nweek_max)
  ) %>%
  as.data.frame()
...
for(i in 1:reps){
  ...
  #   dplyr::group_by(IDy)%>%
  #   sample_n(size = 52*prop_fit) %>% 
  #   pull(IDdata)
  ids_i <- bio.matrix_sel %>%
    dplyr::group_by(IDy) %>%
    dplyr::filter(n() >= 2) %>%          # safety: skip trivially small groups
    dplyr::sample_frac(prop_fit) %>%
    dplyr::pull(IDdata)
...
ggarrange(p_MAE_fit + coord_trans(y='log10'), p_MAE_val, ncol = 1, nrow = 2)
...
cv_summary_full <- bind_rows(
  ...
  RMSLE_fit_ldf  %>% dplyr::rename(value = RMSLE)      %>% mutate(metric = "RMSLE"),
  RMSLE_val_ldf  %>% dplyr::rename(value = RMSLE)      %>% mutate(metric = "RMSLE"),
  MAE_fit_ldf    %>% dplyr::rename(value = MAE)        %>% mutate(metric = "MAE"),
  MAE_val_ldf    %>% dplyr::rename(value = MAE)        %>% mutate(metric = "MAE"),
  ResDev_ldf     %>% dplyr::rename(value = ResDev)     %>% mutate(metric = "ResDev",    dataset = "Fitting"),
  Dispersion_ldf %>% dplyr::rename(value = Dispersion) %>% mutate(metric = "Dispersion", dataset = "Fitting")
)
